...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        is_backlog = not sprint

        if is_backlog and status != Task.STATUS_TODO:
            msg = _('Backlog tasks must have "Not Started" status.')
            raise serializers.ValidationError(msg)
        if started and status == Task.STATUS_TODO:
            msg = _('Stated datae cannot be set for not started tasks.')
            raise serializers.ValidationError(msg)
        if completed and status != Task.STATUS_DONE:
            msg = _('Completed data cannot be set for uncompleted tasks.')
            raise serializers.ValidationError(msg)
